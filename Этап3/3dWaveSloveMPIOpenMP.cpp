...
int main(int argc, char **argv)
{
  ...
  if (provided < MPI_THREAD_FUNNELED) {
      if (rank == 0) {
          cerr << "Error: MPI does not provide required MPI_THREAD_FUNNELED level\n";
      }
  ...
  if (rank == 0)
  {
    ...
    cout << "OpenMP threads per MPI process: " << n_threads << endl;
  }
  ...
  #pragma omp parallel for collapse(3) reduction(max : max_abs_err_local) schedule(static)
  for (int i = 1; i <= Nx_local; i++)
  {
    for (int j = 1; j <= Ny_local; j++)
    {
